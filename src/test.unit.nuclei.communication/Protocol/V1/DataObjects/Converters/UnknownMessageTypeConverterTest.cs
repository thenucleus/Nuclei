...
namespace Nuclei.Communication.Protocol.V1.DataObjects.Converters
{
    ...
    public sealed class UnknownMessageTypeConverterTest
    {
        [Test]
        public void MessageTypeToTranslate()
        {
            var translator = new UnknownMessageTypeConverter();
            Assert.AreEqual(typeof(UnknownMessageTypeMessage), translator.MessageTypeToTranslate);
        }
        ...
        {
            var translator = new UnknownMessageTypeConverter();
            Assert.AreEqual(typeof(UnknownMessageTypeData), translator.DataTypeToTranslate);
        }
        ...
        public void ToMessage()
        {
            ...
            var data = new UnknownMessageTypeData
            {
                Id = new MessageId(),
                InResponseTo = new MessageId(),
                Sender = new EndpointId("a"),
        ...
        public void FromMessageWithNonMatchingMessageType()
        {
            ...
            Assert.IsInstanceOf(typeof(UnknownMessageTypeMessage), data);
        ...
        public void FromMessage()
        {
            ...
            var msg = new UnknownMessageTypeMessage(new EndpointId("a"), new MessageId());
            var data = translator.FromMessage(msg);
            Assert.IsInstanceOf(typeof(UnknownMessageTypeMessage), data);
            Assert.AreSame(msg.Id, data.Id);
            Assert.AreSame(msg.Sender, data.Sender);
