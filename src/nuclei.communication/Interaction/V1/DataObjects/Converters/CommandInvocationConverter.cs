...
namespace Nuclei.Communication.Interaction.V1.DataObjects.Converters
{
    ...
    internal sealed class CommandInvocationConverter : IConvertCommunicationMessages
    {
        ...
        {
            var invocationData = data as CommandInvocationData;
            ...
            try
            {
                var interfaceType = TypeLoader.FromPartialInformation(
                    invocationData.InterfaceType.FullName,
                    invocationData.InterfaceType.AssemblyName);

                var parameterValues = new Tuple<Type, object>[invocationData.ParameterTypes.Length];
                for (int i = 0; i < invocationData.ParameterTypes.Length; i++)
                {
                    var typeInfo = invocationData.ParameterTypes[i];
                    var type = TypeLoader.FromPartialInformation(typeInfo.FullName, typeInfo.AssemblyName);

                    var serializedObjectData = invocationData.ParameterValues[i];
        ...
        {
            var invocationMessage = message as CommandInvokedMessage;
            ...
            try
            {
                var valuePairs = invocationMessage.Invocation.ParameterValues;
                ...
                return new CommandInvocationData
                    {
                        ...
                        InterfaceType = new SerializedType
                            {
                                FullName = invocationMessage.Invocation.Command.InterfaceType.FullName,
                                AssemblyName = invocationMessage.Invocation.Command.InterfaceType.Assembly.GetName().Name
                            },
                        MethodName = invocationMessage.Invocation.Command.MethodName,
